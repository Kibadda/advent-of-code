...
function M:solver(value, cache)
  local registers = Program {
    ...
    hooks = {
      ...
      post = function(p)
        if p.pointer == 29 then
          if cache then
            if cache.seen[p.registers[6]] then
              return true
            else
              cache.seen[p.registers[6]] = true
              cache.last = p.registers[6]
            end
          else
            return true
          end
...
function M:solve1()
  return self:solver(1)[6]
end

function M:solve2()
  local cache = { seen = {}, last = math.huge }
  self:solver(0, cache)
  return cache.last
end
